...
import java.util.Arrays; // <-- BỔ SUNG IMPORT
import java.util.List;
...
public class UserService {
    ...
    // ======================================================
    // === KHU VỰC COMMENT VALIDATE (THEO YÊU CẦU) ===
    // ======================================================
    /*
     * TÓM TẮT CÁC QUY TẮC VALIDATE ĐANG ÁP DỤNG TRONG SERVICE NÀY:
     *
     * 1. Validate Chung (cho nhiều trường):
     * - Không được rỗng (null hoặc blank).
     * - Không quá maxLength (hiện tại là 100 ký tự).
     *
     * 2. Username (Tài khoản):
     * - REGEX: ^[a-zA-Z0-9_.-]*$
     * - Chỉ cho phép chữ, số, và các ký tự an toàn: _ . -
     * - Logic: Phải là duy nhất (unique) trong database.
     *
     * 3. Tên hiển thị & Họ tên:
     * - REGEX: ^[a-zA-Z0-9\\p{L} ]*$
     * - Cho phép chữ cái (kể cả tiếng Việt có dấu), số, và khoảng trắng.
     *
     * 4. Địa chỉ giao hàng:
     * - REGEX: ^[a-zA-Z0-9\\p{L} .,/-]*$
     * - Cho phép chữ cái (tiếng Việt có dấu), số, và các ký tự địa chỉ: . , / -
     *
     * 5. Email:
     ...
     *
     * 7. Các trường bắt buộc (Không null):
     * - Ngày sinh (ngaySinh)
     * - Giới tính (gioiTinh)
     * - ID Role (roleId)
     *
     * 8. Đổi mật khẩu (ChangePasswordRequestDTO):
     * - Mật khẩu mới không được rỗng.
     * - Mật khẩu mới phải có ít nhất 6 ký tự.
     * - Mật khẩu mới và xác nhận mật khẩu phải khớp.
     * - Mật khẩu cũ (oldPassword) phải khớp với mật khẩu đang lưu trong DB.
     */
    // ======================================================
    // === KẾT THÚC KHU VỰC COMMENT VALIDATE ===
    // ======================================================
    ...
    // ===================================
    // === PHƯƠNG THỨC ĐỔI MẬT KHẨU (GIỮ NGUYÊN) ===
    // ===================================
    ...
    // ========================================
    // === CÁC HÀM TÌM KIẾM THEO ROLE (MỚI) ===
    // ========================================

    /**
     * (YÊU CẦU 1)
     * Lấy danh sách user có role = 1 (USER)
     * Yêu cầu: Thêm `List<Users> findByRoleId(Integer roleId);` vào UsersRepository
     */
    public List<UserDTO> findAllUsers_RoleUser() {
        // Role ID 1 = USER (theo database bạn cung cấp)
    ...
    /**
     * (YÊU CẦU 2)
     * Lấy danh sách user có role = 2 (ADMIN) và 3 (STAFF)
     * Yêu cầu: Thêm `List<Users> findByRoleIdIn(Collection<Integer> roleIds);` vào UsersRepository
     */
    public List<UserDTO> findAllUsers_RoleAdminAndStaff() {
        // Role ID 2 = ADMIN, 3 = STAFF (theo database bạn cung cấp)
        List<Integer> roles = Arrays.asList(2, 3);
        return userRepository.findByRoleIdIn(roles)
                .stream()
                .map(this::toDTO)
                .toList();
    }
