...
<template>
  ...
  {{ user.user?.tenHienThi || '' }}
            ...
            <span class="badge bg-gradient text-white px-3 py-2 shadow-sm"
        ...
        <div class="row g-3 mt-4">
          <div class="col-md-6">
            <div class="p-3 bg-light rounded-3 shadow-sm">
              ...
              <p class="mb-0 text-dark">{{ new Date(user.issuedAt).toLocaleString() }}</p>
            </div>
          </div>
          <div class="col-md-6">
            <div class="p-3 bg-light rounded-3 shadow-sm">
              <p class="mb-1 text-secondary fw-semibold">Ngày hết hạn</p>
              <p class="mb-0 text-dark">{{ new Date(user.expiration).toLocaleString() }}</p>
            </div>
          </div>
        </div>
